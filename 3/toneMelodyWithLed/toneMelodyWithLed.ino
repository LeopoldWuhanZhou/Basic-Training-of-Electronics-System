...
int ledCycles[3] = {800, 500, 1100};

void setup() {
  //buzzer output pin setup
  pinMode(buzzerPin, OUTPUT);
  for (int i = 0; i < 2; ++i)
    pinMode(ledPins[i], OUTPUT); // Remember to initialize LED pins
  ...
  for (int thisNote = 0; thisNote < melLength; thisNote++) {
    ...
    for (int t = 0; t < 4; ++t) { // ~0.8 * noteDuration
      tone(buzzerPin, currentNote, 0.2 * noteDuration); //play note with 90% duration
      currentMillis = millis(); 
      for (int i = 0; i < 2; ++i) {
        ledLevel = 127 *(sin((double)(currentMillis % ledCycles[i]) / ledCycles[i]) + 1.0);
    ...
    for (int i = 0; i < 2; ++i) { // last update for LED in this note cycle
        ledLevel = 127 *(sin((double)(currentMillis % ledCycles[i]) / ledCycles[i]) + 1.0);
        analogWrite(ledPins[i], ledLevel);
    }
