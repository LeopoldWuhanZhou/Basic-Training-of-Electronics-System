/*
  转速测量程序
  通过串口发送数据，数据格式：关键字+数据（R123L250,表示右轮驱动电压为123，左轮为250；驱动电压范围：-255～255）
  需要注意的细节：定时器2的使用会影响Pin3和Pin11的PWM输出；同时定时器2中断会影响串口接收数据，故在串口接收之前，先暂停定时器2中断。
 */ 
//  程序执行流程：
//  1、通过串口界面，输入左右后轮驱动电压
//  2、通过10ms定时器，周期性读取左右后轮的转圈计数值
//  3、通过串口输出左右后轮速度（10ms内的转圈计数值）

#include <MsTimer2.h>               //定时中断头文件
#include <PinChangeInterrupt.h>     //外部中断头文件，与讲义中给的示例有区别
...
int  maxPwm = 250, maxVel;                  //maxVel = 47,10ms定时器中断时，最大转速约为47
...
void setup()
{
  ...
  sei();                            //全局中断开启
  attachInterrupt(0, READ_ENCODER_R, CHANGE);             //开启外部中断 右轮编码器    //系统库函数，中断通道0，对应D2引脚；中断通道1,对应D3引脚
...
void loop()
{
  long timer;                   //变量：记录当前时间
  if (stringComplete == true)
  {
    stringComplete = false;
    Serial.print("Command: " + comdata);
  }
  Driver(MotorR, MotorL);     //根据串口接收的命令，驱动两后轮转动
  MsTimer2::start();          //开启定时器中断
  
  timer = millis();           //记录当前时间
  while ((millis() - timer) < 3000) //循环3000ms，回传两后轮转速，以及电池电压
  {
...
void serialEvent()
{
  ...
  while (SerialStart > 0)
  {
    ...
    delay(2);
    data = int(inChar - 48);
    if (data >= 0 && data <= 9)   //数据格式：关键字+数据，例如R123L250，表示MotorR=123，MotorL=250；
    {
      switch (k)
      {
        case 0: break;
        case 1: MotorR = MotorR * 10 + data * sign; break;
        case 2: MotorL = MotorL * 10 + data * sign; break;
        //……      //可以任意定义其他传送通道
        default: break;
      }
    }
    switch (inChar)
    {
      case 'R': k = 1; MotorR = 0; sign = 1; break;
      case 'L': k = 2; MotorL = 0; sign = 1; break;
      //……      //可以任意定义其他传送通道
      case '-': sign = -1; break;
